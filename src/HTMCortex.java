 ...
 */
public class HTMCortex {
    public Integer time = 0;
    public Integer totalTime = 0;

    // Spatial
    public LinkedList<HTMColumn> columns = new LinkedList<HTMColumn>();
    public LinkedList<LinkedList<HTMSynapse>> potentialSynapses = new LinkedList<LinkedList<HTMSynapse>>();
    ...
    // Temporal
    public LinkedList<LinkedList<HTMCell>> cells = new LinkedList<LinkedList<HTMCell>>();
    public LinkedList<LinkedList<LinkedList<HTMSegment>>> dendriteSegments = new LinkedList<LinkedList<LinkedList<HTMSegment>>>();
    ...
    public class segmentUpdate {
        ...
        public LinkedList<HTMSynapse> activeSynapses;
        public Boolean sequenceSegment = false;

        public segmentUpdate(Integer[] segmentIndex, LinkedList<HTMSynapse> activeSynapses) {
    ...
    public LinkedList<Integer> neighbours(Integer c) {
        ...
        HTMColumn currentColumn = this.columns.get(c);
    ...
    public LinkedList<HTMSynapse> connectedSynapses(Integer c) {
        LinkedList<HTMSynapse> result = new LinkedList<HTMSynapse>();
        for(HTMSynapse synapse: this.potentialSynapses.get(c)) {
    ...
    public Double averageReceptiveFieldSize() {
        ...
        for(int i=0;i<this.columns.size();i++) {
            xDistance = 0.0;
            yDistance = 0.0;
            for(HTMSynapse synapse : this.potentialSynapses.get(i)) {
                if (synapse.permanence > this.connectedPerm) {
    ...
    public void increasePermanences(Integer c, Double s) {
        for(HTMSynapse syn: potentialSynapses.get(c)) {
    ...
    public Integer[] getActiveSegment(Integer c, Integer i, Integer t, State state) {
        LinkedList<HTMSegment> activeSegments = new LinkedList<HTMSegment>();
        for(HTMSegment segment: dendriteSegments.get(c).get(i)) {
        ...
        if (activeSegments.size() == 1) {
            ...
            for(HTMSegment segment: activeSegments) {
            ...
            for(int j = 0; j < activeSegments.size(); j++) {
                Integer counter = 0;
                for(HTMSynapse syn: activeSegments.get(j).synapses) {
                    if(list.get(syn.c).get(syn.i) && syn.permanence > connectedPerm) {
                        counter++;
                    }
                }
    ...
    public segmentUpdate getSegmentActiveSynapses(Integer c, Integer i, Integer t, Integer s, Boolean newSynapses) {
        LinkedList<HTMSynapse> activeSynapses = new LinkedList<HTMSynapse>();
        if (s >= 0) {
            for(HTMSynapse syn: dendriteSegments.get(c).get(i).get(s).synapses) {
    ...
    public void adaptSegments(LinkedList<segmentUpdate> segmentList, Boolean positiveReinforcement) {
        for(segmentUpdate segUpd: segmentList) {
            if (segUpd.segmentIndex[2] < 0) {
                ...
                for(HTMSynapse syn: segUpd.activeSynapses) {
                    if (!dendriteSegments.get(segUpd.segmentIndex[0]).get(segUpd.segmentIndex[1])
                            .get(segUpd.segmentIndex[2]).synapses.contains(syn)) {
                        dendriteSegments.get(segUpd.segmentIndex[0]).get(segUpd.segmentIndex[1])
                                .get(segUpd.segmentIndex[2]).synapses.add(syn);
                    }
                }
            }
        }
    ...
    public void SInitialization() {
        for (int i=0;i<xDimension;i++) {
            for (int j=0;j<yDimension;j++) {
                columns.add(new HTMColumn(i, j));
                potentialSynapses.add(new LinkedList<HTMSynapse>());
            }
        }
        ...
        for(int k = 0;k<potentialSynapses.size();k++) {
            HTMColumn column = columns.get(k);
            LinkedList<HTMSynapse> list = potentialSynapses.get(k);
            ...
            for(int i=0;i<xDimension*yDimension;i++) {
                ...
                list.add(new HTMSynapse(dimX, dimY, Math.max(perm - adjustment, 0.0)));
            }
        }
        ...
        for(int c=0;c<xDimension*yDimension;c++) {
            dendriteSegments.add(new LinkedList<LinkedList<HTMSegment>>());
            ...
            for(int i=0;i<cellsPerColumn;i++) {
                ...
                dendriteSegments.get(c).add(new LinkedList<HTMSegment>());
    ...
    public void SOverlap() {
        for(int i=0;i<xDimension*yDimension;i++) {
            overlap[i] = 0.0;
            for(HTMSynapse synapse: connectedSynapses(i)) {
